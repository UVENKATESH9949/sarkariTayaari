...
public interface QuestionRepositoryCustom {
    ...
    Map<String, Long> countGroupedBy(String groupBy, String examCode, UUID subjectId, UUID topicId, String difficulty);

    /** Count of non-deleted questions across a whole set of subjects, for one exam — backs Mock Test's per-section availability. */
    long countForMock(String examCode, List<UUID> subjectIds);

    /** Genuinely random sample (not just "first N") across a set of subjects, for one exam — backs Mock Test's attempt assembly. */
    List<Question> sampleForMock(String examCode, List<UUID> subjectIds, int limit);
}
