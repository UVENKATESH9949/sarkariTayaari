...
| [PREPARATION-PROFILE.md](PREPARATION-PROFILE.md) | What the student told onboarding — name, exam, target year, preparation level, daily study time — as an account-wide row. Device-first and last-write-wins; exists because the daily planner runs on the server |
| [DAILY-PLAN.md](DAILY-PLAN.md) | **What to do today**, in the time the student has. Fills a day from the roadmap and the revision plan without re-ranking either, and is the one part of this program that stores what it produced |
