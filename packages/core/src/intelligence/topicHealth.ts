...
if (Math.abs(WEIGHT_SUM - 1) > 1e-9) {
  ...
  // `typeof` guard, not a bare `__DEV__`: Metro injects that global on React Native but a
  // browser bundle has no such binding, so referencing it directly would throw a
  // ReferenceError here — masking the weight bug this check exists to report. web/ defines
  // __DEV__ through Vite so both platforms get the same loud-in-development behaviour.
  if (typeof __DEV__ !== "undefined" && __DEV__) {
    // Fail immediately and unmissably while developing — this is the check doing its job.
    throw new Error(message);
  }
  // Release: report and carry on. console.error is picked up by Sentry's default integration
  // on both React Native and the browser, so this still reaches the dashboard without this
  // pure module having to import either app's telemetry layer.
