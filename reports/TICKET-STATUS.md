...
| TICKET-2104 PYQ provenance | 🔵 backend + admin + mobile done | Migration V13 adds `is_pyq`/`pyq_year`/`pyq_shift`/`source_paper_id`/`question_number`/`source_url`. Supersedes Epic F/TICKET-1501's narrower version rather than building both. `is_pyq` stored separately from `pyq_year` so "PYQ, year unknown" stays representable. One shared `applyPyqProvenance` across create/update/bulk-import via `PyqProvenanceCarrier` — it clears year/shift/paper/number when the flag is off (a stale year would keep feeding the trend) but deliberately keeps `source_url`. Admin: a PYQ card on the question form with a source-paper picker; PYQ badges in the questions list. Mobile: an "Asked in 2023 · Shift 2" badge in the quiz |
| TICKET-2105 `user_topic_progress` | 🔵 backend + mobile done | Migration V14. **Unblocks Epics A, C and D.** Modelled on `user_bookmarks` (mutable, last-write-wins on `updated_at`), not on the append-only practice tables. `TopicProgressState.canTransitionTo` rejects what a CHECK cannot: NEEDS_REVISION is reachable only from MASTERED, and nothing may return to NOT_STARTED. Bad rows are rejected and counted individually so one unknown topic cannot block a student's other progress. Mobile records mastery on quiz finish (using the real `topicId` from route params — `SessionRecord` only carries `topicName`, which is the §18.2 gap itself) and syncs both ways |
| TICKET-2106 trend/priority + algorithm versioning | 🔵 backend + admin + mobile done | Migration V15. Trend from a two-half comparison of tagged years, not a least-squares slope — with 3–4 tagged years a slope is dominated by whichever year is missing. `INSUFFICIENT_DATA` is a real verdict, not an error. Priority = 0.55 weightage + 0.30 trend + 0.15 coverage, asserted to sum to 1.00 at startup. Every row stores `algorithm_version` + `inputs` (JSONB) per §65/§67. **Real bug: v1's formula did not do what its own weights claimed** — computed weightage came out ~0.5 on a 0–100 scale, so ranking was effectively trend-only; normalised and bumped to `v2` rather than edited in place, which is what per-row versioning is for |
| TICKET-2107 admin priority override | 🔵 backend + admin done | Three separate columns (`system_priority`, `admin_override`, `final_priority`) per §66 — an override never overwrites the computed value. A DB CHECK asserts `final IS NOT DISTINCT FROM COALESCE(override, system)`. `admin_override` nullable because "no override" ≠ "override of 0". A reason is mandatory (service + CHECK) or the decision is unauditable. `recompute` carries overrides forward, else bumping the version would silently discard every editorial decision. New admin page `TopicIntelligence.jsx` shows all three columns side by side |
| TICKET-2108 real pattern versioning | 🔵 backend + admin done | Migration V16 drops `UNIQUE (exam_code, name)` and replaces it with a `COALESCE(version_label, '')` unique index — a plain 3-column UNIQUE would have silently removed duplicate protection for every un-versioned stage, since Postgres treats NULLs as distinct. **The DB change alone was not enough:** `createStage` had its own name-only check, the code-level twin of the constraint. Adds `effective_to`, with resolution in one place (`isEffectiveOn`). The admin endpoint returns every version flagged `active`; the mobile sync endpoint returns only the effective one, so relaxing uniqueness cannot let a device generate a mock test from a superseded pattern |
| TICKET-2109 server-side dedup | 🔵 backend + admin done | Migration V13 adds `content_fingerprint` (backfilled onto ~37,900 rows in SQL) and `question_duplicates`. Checked against the **whole bank**, not just the pasted batch, on create and bulk import. Records the pair and **never deletes** (§14). MD5 is a content fingerprint, not a security choice — it has to match Postgres's built-in `md5()` used by the backfill. New admin page `Duplicates.jsx`. **Found by running it: the live bank holds 2,189 duplicate fingerprint groups**, as expected from the templated load-test content and previously undetectable |
| Synthetic Epic L curation data (requested explicitly) | ✅ seeded and reversible | `SyntheticCurationService`, behind an admin token **and** `app.epic-l.synthetic-seed-enabled` (default false). 61 topic parents, 97 prerequisite edges, 595 topic-map rows, 8,962 PYQ-tagged questions, intelligence for 12 exams, 2 overrides per exam. Deterministic (MD5 of each row's own UUID, not an RNG) so re-runs are identical and idempotent. Year distribution is skewed per topic so trends genuinely vary (32 RISING / 24 FALLING / 5 STABLE on SSC CGL) — a uniform draw would have made every topic STABLE and left TICKET-2106 untested. PYQ rows carry a `synthetic://epic-l-demo` marker so purge removes exactly them |
| Admin console click-tested in a real browser for the first time | ✅ verified via Playwright | The recorded blocker was access, not effort: the only working admin's password is deliberately not in this public repo. `AdminTokenMintRunner` mints a 45-minute token for the **existing ADMIN test fixture** — no human credentials, no password stored, env-var gated so it is invisible to `mvn test`/CI. Confirmed: 61 intelligence rows render, the three priority columns are visibly distinct, client-side validation fires, **the override persisted across a full reload** (the exact check the previous session's shadowed-import bug would have failed), clearing restores the computed value, PYQ field enablement is correct, and **zero console errors** |
| Real bug: `backfillDetection` loaded the entire question bank into memory | ✅ fixed | `findAll()` over ~37,900 entities with lazy associations, to read two columns — the request never returned. Rewritten as one set-based statement using `first_value`/`row_number` over the V13 fingerprint index. Fourth time this codebase has fixed this same shape |
| Real bug: a percent sign in a SQL *comment* threw at runtime | ✅ fixed | The seeder built SQL with `"""…""".formatted(…)`; "the first ~45% of each subject" parsed as an octal format directive (`% o`) and threw `IllegalFormatConversionException`. Rewritten with concatenation so no comment edit can break a query |
| Real bug: override carry-forward could resurrect a cleared override | ✅ fixed | `findOverridesForExam` returned overrides at *any* algorithm version, and superseded versions stay on disk by design — so clearing an override on v2 could be undone by a stale v1 row on the next recompute. Now newest-first, first-hit-per-topic |

Full reports: [20-epic-l-topic-model/epic-l-topic-model.md](./20-epic-l-topic-model/epic-l-topic-model.md) (TICKET-2101/2102/2103) and [21-epic-l-intelligence-and-pyq/epic-l-intelligence-and-pyq.md](./21-epic-l-intelligence-and-pyq/epic-l-intelligence-and-pyq.md) (TICKET-2104–2109).

**Epic L is now complete on the backend and admin side, and mobile consumes it.** Migrations V13–V16 are applied to the shared Neon DB (schema at v16) and Hibernate's `ddl-auto: validate` passes on startup, which is what proves the six new entity mappings match the real schema.

**Not yet verified:** the mobile app has not been launched on an emulator or device since these changes. Local migration `0012` has therefore never executed, and it is the riskiest item — SQLite has no `ADD COLUMN IF NOT EXISTS`, so its five ADD COLUMN statements cannot be guarded, and a failed migration is a hard gate that stops the app starting.
